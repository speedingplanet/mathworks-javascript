...
Either use "classic" style, e.g. getRecords() / setRecords.  
Or accessor/mutator style, e.g. get records() { return this._records }  

Create a `render` method that creates some HTML (placeholder HTML is fine for now) and adds it as the only child of el.

Import DataTable.js into lab-03.js. Create an instance of it and assign/render it somewhere in lab-03.html. Make sure that it renders correctly. (That is, after configuring it, call the `render` method and make sure that it shows up in the page)
